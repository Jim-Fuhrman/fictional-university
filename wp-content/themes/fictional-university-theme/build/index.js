...
class Search {
  ...
  typingLogic() {
    ...
    if (this.searchField.value != this.previousValue) {
      console.log(`typingLogic is executing`); // we clear the timeout with each keystroke. 
  ...
  getResults() {
    ...
    */
    fetch(universityData.root_url + `/wp-json/university/v1/search?term=${this.searchField.value}`).then(response => response.json()).then(results => {
      this.resultsDiv.innerHTML = `
        <div class="row">
          <div class="one-third">
  ...
  keyPressDispatcher(e) {
    ...
    if (s_key_was_pressed && !this.isOverlayOpen && !jquery__WEBPACK_IMPORTED_MODULE_0___default()("input, textarea").is(':focus')) {
      this.openOverlay();
    }
